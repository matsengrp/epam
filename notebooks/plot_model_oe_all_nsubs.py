...
# Stratify PCP data by number of amino acid substitutions (nsubs).
# Also, stratifty data to PCPs of ancestral nodes ('anc') and PCP where child is a leaf node ('leaf').
# Generates CSV file of model performance metrics for each dataset and stratification.
...
model_list = [
    ...
    "ThriftyHumV0.2-59", 
    "ThriftyProdHumV0.2-59", "ThriftyESM_mask", #"ThriftyBLOSUM",
...
modelname_list = [
    ...
    "Thrifty-SHM", 
    "Thrifty-prod", "Thrifty-SHM + ESM-1v", #"Thrifty-SHM + BLOSUM62",
...
for pcptype in ['','anc','leaf']:
    ...
    for dsinfo in dataset_list:
        ...
        for model, modelname in zip(model_list, modelname_list):
            print("Model:", model)
            full_sitemuts_df = pd.read_csv(f'{dfs_dir}/{dsname}_{model}_ssp_df.csv.gz', index_col=0, dtype={'site':'object'})
            full_muts_obs_pred_df = pd.read_csv(f'{dfs_dir}/{dsname}_{model}_site_subs_df.csv.gz', index_col=0, dtype={'site':'object'})
            full_subacc_df = pd.read_csv(f'{dfs_dir}/{dsname}_{model}_site_subacc_df.csv.gz', index_col=0, dtype={'site':'object'})
            full_csp_df = pd.read_csv(f'{dfs_dir}/{dsname}_{model}_csp_df.csv.gz', index_col=0, dtype={'site':'object'})
            
            nmuts_df = full_muts_obs_pred_df.groupby('pcp_index').sum('obs')
            
            for nsubs in range(1,26):
                
                keep_indices = nmuts_df[nmuts_df['obs']==nsubs].index.to_numpy()
                ...
                sitemuts_df = full_sitemuts_df[full_sitemuts_df['pcp_index'].isin(keep_indices)].copy()
                ...
                muts_obs_pred_df = full_muts_obs_pred_df[full_muts_obs_pred_df['pcp_index'].isin(keep_indices)]
                ...
                #
                subacc_df = full_subacc_df[full_subacc_df['pcp_index'].isin(keep_indices)]
                subacc_results = plot_sites_subs_acc(subacc_df, None, ax2, numbering)
                subaccs = subacc_results['site_subacc']
                
                csp_df = full_csp_df[full_csp_df['pcp_index'].isin(keep_indices)]
                ...
                csp_df = csp_df[csp_df['prob']!=0].copy()
                ...
                fig.suptitle(f'{dstitle}, {modelname}, (N={nsubs})', fontsize=20, x=0.12, ha='left')
                
                outfname = f"{output_dir}/{dsname}_{model}_nsubs{nsubs}_oe"
