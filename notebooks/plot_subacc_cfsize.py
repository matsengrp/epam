...
df = pd.read_csv(f'{dsname}_nsubs_cfsize_subacc.csv')

cfsizecats = ['cf2','cf3','cf4','cf5_6','cf7_10','cf11_20','cf21_50','cfmax']
xticks = np.arange(len(cfsizecats))
xticklabels = ['2','3','4','5-6','7-10','11-20','21-50','50+']
...
for nsubs in [1,2,3]:
    yvals[nsubs] = []
    npcps[nsubs] = []
    for cat in cfsizecats:
        yvals[nsubs].append(df[(df['nsubs']==nsubs) & (df['cfsize']==cat)]['All'].item())
        npcps[nsubs].append(df[(df['nsubs']==nsubs) & (df['cfsize']==cat)]['nPCP'].item())
...
axs[-1].set_xlabel("CF size", fontsize=26)
...
outfname = f"{output_dir}/subacc_vs_cfsize"
