...
output_df = pd.DataFrame(columns=['model','nsubs','All','FWR1','CDR1','FWR2','CDR2','FWR3','CDR3','FWR4'])
...
for pcptype in ['','anc','leaf']:
    ...
    for model, modelname in zip(model_list, modelname_list):
        ...
        if pcptype=='anc':
            pcp_df = pcp_df[pcp_df['child_is_leaf']==False]
        elif pcptype=='leaf':
        ...
        with h5py.File(aaprob_path, "r") as matfile:
            for index in range(len(parent_aa_seqs)):
                ...
                if nsubs<1 or nsubs>25:
                    continue
                
                pcp_sub_correct[nsubs].append(
                    [
                        child_aa[j]
        ...
        for i in range(1,26):
            ...
            df['correct'] = np.concatenate(pcp_sub_correct[i])
            df['region'] = np.concatenate(pcp_regions[i])
    ...
    print(output_df)
    if len(pcptype)>0:
        outfname = f"{dsname}_{pcptype}_nsubs_subacc.csv"
    else:
        outfname = f"{dsname}_nsubs_subacc.csv"
